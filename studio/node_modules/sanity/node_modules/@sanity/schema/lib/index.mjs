...
function parsePreview(preview) {
  ...
  return Array.isArray(select) ? __spreadProps$4(__spreadValues$6({}, pick(preview, ["prepare", "component"])), {
    select: parseSelection(select)
  }) : __spreadProps$4(__spreadValues$6({}, pick(preview, ["prepare", "component"])), {
    select
  });
}
...
const INHERITED_FIELDS = [
  ...
  extend(subTypeDef, extendMember) {
    const options = __spreadValues$4({}, subTypeDef.options || DEFAULT_OPTIONS$2), { annotations = [], marks = [] } = subTypeDef, fields = [MARKS_FIELD, TEXT_FIELD], parsed = Object.assign(pick(SPAN_CORE, INHERITED_FIELDS), subTypeDef, {
      type: SPAN_CORE,
      options
    });
    return lazyGetter(parsed, "fields", () => fields.map((fieldDef) => {
...
const CrossDatasetReferenceType = {
  ...
  extend(subTypeDef, createMemberType) {
    ...
    })), lazyGetter(parsed, "to", () => arrify(subTypeDef.to).map((toType) => __spreadProps$3(__spreadValues$3({}, toType), {
...
const OVERRIDABLE_FIELDS$8 = [
  ...
  extend(rawSubTypeDef, createMemberType) {
    const subTypeDef = __spreadValues$2({ fields: [] }, rawSubTypeDef), options = __spreadValues$2({}, subTypeDef.options || {}), parsed = Object.assign(pick(this.get(), OVERRIDABLE_FIELDS$8), subTypeDef, {
      type: this.get(),
...
function createFieldsets(typeDef, fields) {
  ...
  for (const fieldset of typeDef.fieldsets || []) {
    ...
    fieldsetsByName[fieldset.name] = __spreadProps$2(__spreadValues$2({ title: startCase(fieldset.name) }, fieldset), { fields: [] });
  }
...
function createFieldsGroups(typeDef, fields) {
  ...
  for (const group of typeDef.groups || []) {
    ...
    if (groupsByName[group.name] = __spreadProps$2(__spreadValues$2({ title: startCase(group.name) }, group), { fields: [] }), group.default && ++numDefaultGroups > 1)
      throw new Error(
...
const ASSET_FIELD$1 = {
  ...
  extend(rawSubTypeDef, createMemberType) {
    const options = __spreadValues$1({}, rawSubTypeDef.options || DEFAULT_OPTIONS$1), fields = [ASSET_FIELD$1, ...rawSubTypeDef.fields || []], subTypeDef = __spreadProps$1(__spreadValues$1({}, rawSubTypeDef), { fields }), parsed = Object.assign(pick(FILE_CORE, OVERRIDABLE_FIELDS$6), subTypeDef, {
      type: FILE_CORE,
...
const OVERRIDABLE_FIELDS$5 = [...DEFAULT_OVERRIDEABLE_FIELDS], IMAGE_CORE = {
  ...
  extend(rawSubTypeDef, createMemberType) {
    ...
    const fields = [ASSET_FIELD, ...hotspotFields, ...rawSubTypeDef.fields || []], subTypeDef = __spreadProps(__spreadValues({}, rawSubTypeDef), { fields }), parsed = Object.assign(pick(this.get(), OVERRIDABLE_FIELDS$5), subTypeDef, {
      type: IMAGE_CORE,
