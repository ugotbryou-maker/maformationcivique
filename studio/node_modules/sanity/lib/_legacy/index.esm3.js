...
import { useState, useEffect, useMemo, memo, lazy, Suspense, Fragment } from "react";
...
import { generateHelpUrl, Button, ErrorBoundary } from "./generate-help-url.esm.esm.js";
...
async function resolveIntent(options) {
  const resolvedPaneCache = /* @__PURE__ */ new Map(), resolvePane = createPaneResolver((nextFn) => (unresolvedPane, context, flatIndex) => {
    ...
    params: __spreadProps$2(__spreadValues$2({}, omit(options.params, ["id"])), {
...
function useRouterPanesStream() {
  const routerStateSubject = useMemo(() => new ReplaySubject(1), []), routerPanes$ = useMemo(() => routerStateSubject.asObservable().pipe(map((_routerState) => (_routerState == null ? void 0 : _routerState.panes) || [])), [routerStateSubject]), {
    state: routerState
  } = useRouter();
  return useEffect(() => {
...
`, isSaveHotkey = isHotkey("mod+s"), StructureTool = memo(function(t0) {
  ...
  }, t7 = [pushToast], $[8] = pushToast, $[9] = t6, $[10] = t7) : (t6 = $[9], t7 = $[10]), useEffect(t6, t7), !((_a = schema._original) != null && _a.types.some(_isCustomDocumentTypeDefinition))) {
    let t82;
