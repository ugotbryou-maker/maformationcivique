...
var __defProp$8 = Object.defineProperty, __getOwnPropSymbols$7 = Object.getOwnPropertySymbols, __hasOwnProp$7 = Object.prototype.hasOwnProperty, __propIsEnum$7 = Object.prototype.propertyIsEnumerable, __defNormalProp$8 = (obj, key, value) => key in obj ? __defProp$8(obj, key, { enumerable: !0, configurable: !0, writable: !0, value }) : obj[key] = value, __spreadValues$7 = (a, b) => {
  for (var prop in b || (b = {}))
    __hasOwnProp$7.call(b, prop) && __defNormalProp$8(a, prop, b[prop]);
  if (__getOwnPropSymbols$7)
    for (var prop of __getOwnPropSymbols$7(b))
      __propIsEnum$7.call(b, prop) && __defNormalProp$8(a, prop, b[prop]);
  return a;
};
function promptForDatasetName(prompt, options = {}) {
  return prompt.single(__spreadValues$7({
    type: "input",
...
var __defProp$7 = Object.defineProperty, __defNormalProp$7 = (obj, key, value) => key in obj ? __defProp$7(obj, key, { enumerable: !0, configurable: !0, writable: !0, value }) : obj[key] = value, __publicField = (obj, key, value) => __defNormalProp$7(obj, typeof key != "symbol" ? key + "" : key, value);
class PaginatedGetBackupStream extends node_stream.Readable {
  constructor(client, projectId, datasetName, backupId, token) {
    super({
      objectMode: !0
    }), __publicField(this, "cursor", ""), __publicField(this, "totalFiles", 0), this.client = client, this.projectId = projectId, this.datasetName = datasetName, this.backupId = backupId, this.token = token;
  }
  async _read() {
    try {
...
var __knownSymbol$1 = (name, symbol) => (symbol = Symbol[name]) ? symbol : Symbol.for("Symbol." + name), __forAwait$1 = (obj, it, method) => (it = obj[__knownSymbol$1("asyncIterator")]) ? it.call(obj) : (obj = obj[__knownSymbol$1("iterator")](), it = {}, method = (key, fn) => (fn = obj[key]) && (it[key] = (arg) => new Promise((yes, no, done) => (arg = fn.call(obj, arg), done = arg.done, Promise.resolve(arg.value).then((value) => yes({ value, done }), no)))), method("next"), method("return"), it);
...
const downloadBackupCommand = {
  ...
  action: async (args, context) => {
    ...
    try {
      ...
      let i = 0;
      try {
        for (var iter = __forAwait$1(backupFileStream), more, temp, error; more = !(temp = await iter.next()).done; more = !1) {
...
const DEFAULT_LIST_BACKUP_LIMIT = 30, helpText$w = `
Options
...
function promptForDatasetAliasName(prompt, options = {}) {
  return prompt.single(__spreadValues$5({
    type: "input",
    message: "Alias name:",
...
const exportDatasetCommand = {
  ...
  action: async (args, context) => {
    ...
    try {
      await exportDataset__default.default(__spreadValues$4({
        client,
        dataset,
        outputPath,
        onProgress
      }, flags)), spinner.succeed();
    } catch (err) {
...
function colorizeJson(input, chalk) {
  ...
  return tokenize__default.default(json).map((token, i, arr) => {
    ...
    return token.type === "string" && prevToken.type === "whitespace" && /^\n\s+$/.test(prevToken.value) ? __spreadProps$2(__spreadValues$3({}, token), {
      type: "key"
    }) : token;
...
${err.message}`);
    ...
    const groupedAttempts = groupBy__default.default(attempts, "messageId"), populated = messages.map((msg) => __spreadProps$1(__spreadValues$2({}, msg), {
...
function convertToTree(nodes) {
  ...
  function addNode(node2, tree = root) {
    ...
    tree.children || (tree.children = {}), key in tree.children || (tree.children[key] = {}), addNode(__spreadProps(__spreadValues$1({}, node2), {
      path: rest
    }), tree.children[key]);
  }
...
function mutationHeader(chalk, mutation, migration) {
  var _a;
  const mutationType = badge(mutation.type, mutationImpact[mutation.type], chalk), documentType = "document" in mutation || migration.documentTypes ? badge("document" in mutation ? mutation.document._type : listFormatter.format((_a = migration.documentTypes) != null ? _a : []), "info", chalk) : null;
...
var __knownSymbol = (name, symbol) => (symbol = Symbol[name]) ? symbol : Symbol.for("Symbol." + name), __forAwait = (obj, it, method) => (it = obj[__knownSymbol("asyncIterator")]) ? it.call(obj) : (obj = obj[__knownSymbol("iterator")](), it = {}, method = (key, fn) => (fn = obj[key]) && (it[key] = (arg) => new Promise((yes, no, done) => (arg = fn.call(obj, arg), done = arg.done, Promise.resolve(arg.value).then((value) => yes({ value, done }), no)))), method("next"), method("return"), it);
const helpText$6 = `
Options
...
`), flags.confirm && !await prompt.single({
  ...
  ${transaction && !progress2.done ? `\xBB ${prettyFormat({
    ...
    async function dryRunHandler() {
      ...
      try {
        for (var iter = __forAwait(migrate.dryRun({
          api: apiConfig,
          exportPath: fromExport
        }, migration)), more, temp, error; more = !(temp = await iter.next()).done; more = !1) {
...
`, listUsersCommand = {
  ...
  action: async (args, context) => {
    ...
    const client = apiClient(), globalClient = client.clone().config({
      ...
      useGlobalApi
    }).then((user) => Array.isArray(user) ? user : [user]), members = [...project.members.map((member) => __spreadValues(__spreadValues({}, member), getUserProps(users.find((candidate) => candidate.id === member.id)))).filter((member) => !member.isRobot || robots), ...pendingInvitations], ordered = sortBy__default.default(members.map(({
      id,
      name,
      role,
      date
