...
async function buildVendorDependencies({
  ...
  for (const [packageName, ranges] of Object.entries(VENDOR_IMPORTS)) {
    ...
    try {
      ...
      throw typeof (e == null ? void 0 : e.message) == "string" ? (e.message = `${message}: ${e.message}`, e) : new Error(message, {
        cause: e
      });
    }
...
async function compareStudioDependencyVersions(autoUpdatesImports, workDir, fetchFn = globalThis.fetch) {
  const manifest = checkRequiredDependencies.readPackageJson(path__default.default.join(workDir, "package.json")), dependencies = __spreadValues$1(__spreadValues$1({}, manifest.dependencies), manifest.devDependencies), failedDependencies = [], filteredAutoUpdatesImports = Object.entries(autoUpdatesImports).filter(([pkg]) => !pkg.endsWith("/"));
...
async function buildSanityStudio(args, context, overrides) {
  ...
  } = context, flags = __spreadValues({
    minify: !0,
    stats: !1,
    "source-maps": !1
  }, args.extOptions), unattendedMode = !!(flags.yes || flags.y), defaultOutputDir = path__default.default.resolve(path__default.default.join(workDir, "dist")), outputDir = path__default.default.resolve(args.argsWithoutOptions[0] || defaultOutputDir);
  ...
  const autoUpdatesEnabled = shouldAutoUpdate({
    flags,
    cliConfig
  }), coercedSanityVersion = (_a = semver__default.default.coerce(installedSanityVersion)) == null ? void 0 : _a.version;
...
Including the following environment variables as part of the JavaScript bundle:`), envVarKeys.forEach((key) => output.print(`- ${key}`)), output.print(""));
  ...
  overrides != null && overrides.basePath ? basePath = overrides.basePath : envBasePath ? basePath = envBasePath : configBasePath && (basePath = configBasePath), envBasePath && configBasePath && output.warn(`Overriding configured base path (${configBasePath}) with value from environment variable (${envBasePath})`);
  let spin;
  if (shouldClean) {
  ...
  autoUpdatesEnabled && (importMap = {
    imports: __spreadValues(__spreadValues({}, await buildVendorDependencies({
      cwd: workDir,
      outputDir,
      basePath
    })), autoUpdatesImports)
