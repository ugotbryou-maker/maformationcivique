...
function checkStudioDependencyVersions(workDir) {
  const manifest = readPackageJson$1(path__default.default.join(workDir, "package.json")), dependencies = __spreadValues$1(__spreadValues$1({}, manifest.dependencies), manifest.devDependencies), installedPackages = PACKAGES.map((pkg) => {
    ...
    return __spreadProps(__spreadValues$1({}, pkg), {
      installed,
      isUnsupported,
      isDeprecated,
      isUntested
...
async function readPackageManifest(packageJsonPath, defaults = {}) {
  let manifest;
  try {
    manifest = __spreadValues(__spreadValues({}, defaults), await readPackageJson(packageJsonPath));
  } catch (err) {
