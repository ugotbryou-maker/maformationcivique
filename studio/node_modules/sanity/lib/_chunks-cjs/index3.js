...
async function resolveIntent(options) {
  const resolvedPaneCache = /* @__PURE__ */ new Map(), resolvePane = createPaneResolver((nextFn) => (unresolvedPane, context, flatIndex) => {
    ...
    params: __spreadProps$2(__spreadValues$2({}, omit__default.default(options.params, ["id"])), {
  ...
  async function traverse({
    ...
    const _a = params, {
      ...
      "id",
      "type"
    ]), resolvedPane = await rxjs.firstValueFrom(resolvePane(unresolvedPane, {
      id: currentId,
      splitIndex: 0,
      parent: parent2,
      path,
...
function useRouterPanesStream() {
  const routerStateSubject = React.useMemo(() => new rxjs.ReplaySubject(1), []), routerPanes$ = React.useMemo(() => routerStateSubject.asObservable().pipe(operators.map((_routerState) => (_routerState == null ? void 0 : _routerState.panes) || [])), [routerStateSubject]), {
    state: routerState
  } = router.useRouter();
...
function useResolvedPanes() {
  ...
  return $[1] !== rootPaneNode || $[2] !== routerPanesStream || $[3] !== structureContext ? (t1 = () => {
    const subscription = createResolvedPaneNodeStream({
      ...
      routerPanesStream,
      structureContext
    }).pipe(operators.map(_temp4)).subscribe({
...
function StructureError(t0) {
  ...
  const {
    ...
    t
  } = sanity.useTranslation(structure.structureLocaleNamespace), stack = (cause == null ? void 0 : cause.stack) || error.stack, showStack = stack && !(cause instanceof structure.SerializeError) && !error.message.includes("Module build failed:");
  let t1;
...
const DocumentTitle = (props) => {
  const $ = reactCompilerRuntime.c(7), {
    ...
    t
  } = sanity.useTranslation(structure.structureLocaleNamespace), isNewDocument = !(editState != null && editState.published) && !(editState != null && editState.draft), documentValue = (editState == null ? void 0 : editState.draft) || (editState == null ? void 0 : editState.published), schemaType = schema.get(documentType), {
    value,
...
`, isSaveHotkey = isHotkeyEsm.isHotkey("mod+s"), StructureTool = React.memo(function(t0) {
  ...
  }, t7 = [pushToast], $[8] = pushToast, $[9] = t6, $[10] = t7) : (t6 = $[9], t7 = $[10]), React.useEffect(t6, t7), !((_a = schema._original) != null && _a.types.some(sanity._isCustomDocumentTypeDefinition))) {
    let t82;
