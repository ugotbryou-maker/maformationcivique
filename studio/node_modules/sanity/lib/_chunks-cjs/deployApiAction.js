...
function extractFromSanitySchema(sanitySchema, extractOptions = {}) {
  ...
  function convertType(type, parent, props = {}) {
    ...
    return __spreadValues$2(__spreadValues$2(__spreadValues$2(__spreadValues$2(__spreadValues$2({}, getDeprecation$1(type.type)), props), mapped), original), crossDatasetReferenceMetadata && {
  ...
  function getObjectDefinition(def, parent) {
    ...
    return {
      ...
      fields: objectFields.map((field) => isArrayOfBlocks(field) ? buildRawField(field, name) : convertType(field, name, __spreadValues$2({
        fieldName: field.name
      }, getDeprecation$1(def)))),
      [internal]: __spreadValues$2({}, getDeprecation$1(def))
    };
  }
  function buildRawField(field, parentName) {
    return __spreadProps(__spreadValues$2({}, convertType(field, parentName, {
  ...
  function getReferenceDefinition(def, parent) {
    ...
    return __spreadValues$2(__spreadValues$2({}, unionDefinition), base);
  }
  function getArrayDefinition(def, parent, options = {}) {
    ...
    } : {}, candidates = (_a2 = def.type) != null && _a2.type && "of" in def.type ? arrayify(def.type.of) : def.of;
    return candidates.length === 1 ? __spreadValues$2(__spreadValues$2({
      children: getArrayChildDefinition(candidates[0], def)
    }, base), name) : __spreadValues$2(__spreadValues$2({
  ...
  function getArrayChildDefinition(child, arrayDef) {
    ...
    return isReference(child) ? getReferenceDefinition(child, arrayDef.name) : scalars.includes(child.jsonType) && !scalars.includes(child.name) ? {
  ...
  function getDocumentDefinition(def) {
    ...
    return __spreadProps(__spreadValues$2({}, objectDef), {
      fields,
      interfaces: ["Document"]
    });
  }
  ...
  function getDocumentInterfaceFields(type) {
    ...
    }, getDeprecation$1(type)), __spreadValues$2({
      fieldName: "_type",
      type: "String",
      isNullable,
...
function createFieldFilters(objectType, options) {
  ...
  return objectType.fields ? objectType.fields.filter((field) => field.type !== "JSON" && field.kind !== "List" && !unionTypes.includes(field.type)).map((field) => {
    const typeName = typeAliases[field.type] || field.type, filterSuffix = Object.keys(__spreadValues$1(__spreadValues$1({}, typeAliases), filterCreators)).includes(typeName) ? void 0 : options.filterSuffix;
    return {
      fieldName: field.fieldName,
